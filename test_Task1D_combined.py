...
def test_rivers_with_station():
    ...
    s4 = MonitoringStation("id4", "m4", "Station D", (0,0), (0,1), None, "Town4")  # no river
    ...
    # Assertions (checks)
    assert "River X" in rivers  # River X should appear
    assert "River Y" in rivers  # River Y should appear
    assert None not in rivers   # Stations with no river should be ignored
...
def test_stations_by_river():
    ...
    assert None not in river_dict  # No river key for stations with None

    # Check stations are grouped correctly
